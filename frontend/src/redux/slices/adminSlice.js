  ...
  async (_, { rejectWithValue }) => {
    try {
      ...
      return rejectWithValue(
        error.response?.data || { message: "Failed to featch users" }
      );
    }
  }
);
...
export const addUsers = createAsyncThunk(
  ...
  async (userData, { rejectWithValue }) => {
    try {
      const response = await axios.post(`${API_URL}/api/admin/create`, userData, {
        headers: { Authorization: getAuthToken() },
      });
      return response.data;
    } catch (error) {
...
const adminSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(featchUsers.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload?.message || "Failed to featch users";
      })
